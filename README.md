...
| バイナリ | 用途 | 実行時依存 |
|---|---|---|
| `errsweep` | CLI 解析（本体） | なし |
| `errsweep-lsp-proxy` | gopls の hover に Sentinel 情報を追記する LSP プロキシ | `errsweep`, `gopls` |
| `errsweep-lsp` | スタンドアロン LSP（hover のみ） | `errsweep` |
| `errsweep-profile` | CPU/メモリプロファイリング用ツール（開発・性能測定） | なし |

`errsweep-lsp` / `errsweep-lsp-proxy` は `errsweep` バイナリを子プロセスとして起動するため、
LSP 構成でも `errsweep` 本体が必要です。

`errsweep-profile` はベンチマークと性能分析を目的とした開発ツールです。通常の CLI/LSP 利用では不要です。

## インストール

```bash
go install github.com/YukiYuigishi/errsweep/cmd/...@latest
```

`cmd/...` ワイルドカードで 3 バイナリがまとめて `$GOBIN` に入ります。
LSP proxy を使う場合は別途 `gopls` も必要です。

```bash
go install golang.org/x/tools/gopls@latest
```

CLI のみで良い場合は個別に指定することもできます。
...
`--cache-file` を省略した場合、キャッシュは OS の一時ディレクトリ配下に workspace ごとに保存されます。永続的な保存先を固定したい場合だけ明示指定してください。
