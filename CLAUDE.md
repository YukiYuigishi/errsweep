...
.
├── cmd/
│   ├── errsweep/        # スタンドアロンCLI（singlechecker/multichecker）
│   │   └── main.go
│   ├── errsweep-lsp/    # LSP server（gopls hover 完全置き換え）
│   │   └── main.go
│   ├── errsweep-lsp-proxy/  # LSP proxy（gopls の上に重ねる）
│   │   └── main.go
│   └── errsweep-profile/    # CPU/memory profiling tool（開発・ベンチマーク）
│       └── main.go
├── analyzer/
...
# スタンドアロンCLIビルド・実行
go build -o errsweep ./cmd/errsweep
./errsweep ./path/to/target/package
