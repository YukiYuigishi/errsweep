...
- interface invoke 解決の強化（現状の compile-time assertion 依存を補完）
  - RTA/callgraph を使った候補具象の推定
  - 過剰推論を避ける保守的モードとの両立
- known error map の拡張と保守運用設計
  - `net`, `syscall`, `context` など周辺 API への対応方針
- `%w` 以外のラップ表現を「同一性喪失」として明示的に扱う診断方針整理

**完了条件**

- 代表ケースでの誤検知/見逃し傾向が可視化され、改善サイクルが回る状態
- interface 経由ケースの検出率向上がテストで確認できる状態
