...
- `errsweep-lsp-proxy`（gopls hover 応答へ Sentinel 情報を追記）
- `errsweep-lsp`（hover 専用ミニマル LSP）
...
- キャッシュ更新の粒度改善（差分再解析） — **進行中**
  - `errsweep-lsp-proxy` に LSP ファイル変更通知（didSave / watchedFiles）トリガーの自動キャッシュ再構築を導入（debounce付き）
  - Go関連ファイル（`.go`, `go.mod`, `go.sum`, `go.work`）のみ再解析対象に絞り込み
  - workspace 外ファイルの通知は再解析対象から除外
  - 連続保存時の過剰再構築を避ける最小間隔スロットル（`--cache-refresh-min-interval`）を追加
- 解析結果キャッシュ形式の改善 — **実装済み**
  - `errsweep-lsp-proxy` に `--cache-pattern` を追加し、大規模リポジトリで対象パッケージを絞ったキャッシュ構築を可能化
  - `proxy` に gob 形式の永続キャッシュ保存/復元を導入（`SaveCacheToFile` / `LoadCacheFromFile`）
  - `errsweep-lsp-proxy` に `--cache-file` を追加（既定: OS の一時ディレクトリ内の workspace 別 cache.gob）
  - `errsweep` 失敗/タイムアウト時は永続キャッシュへフォールバックして hover 継続性を確保
- 大規模リポジトリ向けの計測・ボトルネック分析
  - `errsweep-profile` による CPU/メモリプロファイル取得（開発ツール）
  - `errsweep-lsp-proxy` に初回/再構築の所要時間ログを追加し、運用時のボトルネック観測を可能化
