...
- キャッシュ更新の粒度改善（差分再解析）
  - `sentinel-lsp-proxy` に LSP ファイル変更通知（didSave / watchedFiles）トリガーの自動キャッシュ再構築を導入（debounce付き）
...
- 解析結果キャッシュ形式の改善（JSON中心 → 必要なら SQLite 併用）
  - `sentinel-lsp-proxy` に `--cache-pattern` を追加し、大規模リポジトリで対象パッケージを絞ったキャッシュ構築を可能化
  - `proxy` に gob 形式の永続キャッシュ保存/復元を導入（`SaveCacheToFile` / `LoadCacheFromFile`）
  - `sentinel-lsp-proxy` に `--cache-file` を追加（既定: `<workspace>/.errsweep/cache.gob`）
  - `sentinelfind` 失敗/タイムアウト時は永続キャッシュへフォールバックして hover 継続性を確保
- 大規模リポジトリ向けの計測・ボトルネック分析
  - `sentinel-lsp-proxy` に初回/再構築の所要時間ログを追加し、運用時のボトルネック観測を可能化
